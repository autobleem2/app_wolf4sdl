...
| `patches/wolf4sdl/0002-menu-cross-accepts.patch` | **Linux only** (the virtual pad's PlayStation Classic layout): the menus (`ReadAnyControl`, which only they read) take Cross as accept/yes, Circle as back/no, Square as backspace in a name, Triangle last - upstream's button 0 accept was Triangle there (the owner, 2026-09-25, from `20260504-2`). Windows keeps upstream's order, where XInput's A is button 0. The game's own buttons are `buttonjoy[]` (0001), untouched |
